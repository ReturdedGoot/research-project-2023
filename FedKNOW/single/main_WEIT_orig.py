...
if __name__ == '__main__':
    ...
    if 'cifar' in args.dataset or args.dataset == 'mnist' or 'miniimagenet' in args.dataset or 'FC100' in args.dataset or 'Corn50' in args.dataset:
        ...
        for idx in dict_users_train.keys():
            np.random.shuffle(dict_users_train[idx]) #shuffle tasks within a client??
    ...
    for user in range(args.num_users):
        ...
        for key in net_glob.state_dict().keys():
            w_local_dict[key] = net_glob.state_dict()[key] #for each client create a local dictionary to access each of his model's layers by name
