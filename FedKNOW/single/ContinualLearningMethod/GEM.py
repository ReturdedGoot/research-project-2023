...
class Appr():
    def __init__(self,
        ...
        self.memory_data = torch.LongTensor(
    ...
    def observe(self, x, t, y):
        ...
        # print(f'x_data shape: {x.data[: effbsz].view(effbsz,-1).size()} ')
        # print(f'memory_data: { self.memory_data[t, self.mem_cnt: endcnt].size()}')
        ...
        loss = self.ce(self.net.forward(x, t), y)
        loss.backward()
...
def life_experience(task,appr,tr_dataloader,epochs,sbatch=10):
    ...
    for e in range(epochs):
        for count, (images,targets) in enumerate(tr_dataloader):
            # targets.apply_(lambda x: int(labelMapper(x) + 5*task))
            # if torch.cuda.is_available():
            #     images = images.cuda()
            #     targets = targets.cuda()
            appr.net.train()
            # print(f'text size: {images.size()} ')
            loss = appr.observe(images, task, targets)
...
def LongLifeTestCustom(args, appr, t, testdatas, writer):
    ...
    for u in range(t + 1):
        ...
        test_loss, test_acc = appr.validTest(u, tr_dataloader)
