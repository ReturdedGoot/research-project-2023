...
from FedKNOW.utils.sampling import noniid, specnoniid
import os
import numpy as np
import json
...
def get_data(args):
    if args.dataset == 'cifar100':
        ...
        if args.partitioning != "":
            cifar100 = Cifar100Task(args.root,task_num=20,data_transform=data_transform, download=args.download, labelType=args.labelType)
        else:
            cifar100 = Cifar100Task(args.root,task_num=10,data_transform=data_transform, download=args.download, labelType=args.labelType)
        dataset_train,dataset_test = cifar100.getTaskDataSet()
        if(args.partitioning == ""): #if its the default partitioning mechanism
            ...
            if(args.task > 20):
                logging.error("Maximum limit of 20 tasks for Cifar100!")
                exit(0)
            elif args.partitioning != "unique":
                tasks = np.random.permutation(20)[:args.task] #gets a sequence of num_task to schedule per-client.
            else:
                try:
                    tasks = np.random.permutation(20)[:args.task * args.num_users]
                    assert(len(tasks) == args.task * args.num_users)
                except:
                    logging.error("Too many clients for the current number of tasks. Can't guarantee uniqueness")
                    exit(0)
            dict_users_train, rand_set_all = specnoniid(dataset_train, args.num_users, args.task, args.partitioning, tasks)
            dict_users_test, rand_set_all = specnoniid(dataset_test, args.num_users, args.task, args.partitioning, tasks)
            if(args.partitioning == "shuffled"):
                zipped_dict = {}
                for client in range(args.num_users):
                    zipped_train_test = list(zip(dict_users_train[client], dict_users_test[client]))
                    np.random.shuffle(zipped_train_test)
                    # random.shuffle(zipped_train_test)
                    dict_users_train[client], dict_users_test[client] = zip(*zipped_train_test)
...
def get_model(args):
    ...
    if args.model == '6layer_CNN' :
        ...
        if(args.alg.lower() != 'weit'):
            ...
            net_glob = Cifar100WEIT(image_size,n_ouputs=args.num_classes,nc_per_task=5, window = args.softmaxtype).to(args.device)
        ...
        if 'cifar100' in args.dataset or 'FC100' in args.dataset or 'CORe50' in args.dataset:
            ...
            image_size = [3, 224, 224]
        if(args.alg.lower() == 'weit'):
            net_glob = LeNetWEIT(image_size,n_ouputs=args.num_classes,nc_per_task=5, window = args.softmaxtype).to(args.device)
        else:
            net_glob = LeNet(image_size, n_outputs=args.num_classes, nc_per_task=5, window = args.softmaxtype).to(args.device)
