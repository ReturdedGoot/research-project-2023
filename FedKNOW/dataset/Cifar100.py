...
class Cifar100Task():
    def __init__(self, root, task_num=1, data_transform=None, download=False, labelType="coarse"):
        self.root = root
    ...
    def getTaskDataSet(self):
        trainDataset = myCIFAR100(root=self.root, train=True, transform=self.transform_train, download=self.download,
                                  task_num=self.task_num, labels = self.labelType)
        train_task_datasets = [CifarDataset(data, self.transform_train) for data in
                               trainDataset.task_datasets]  # the training samples partitioned into 10 tasks
        testDataset = myCIFAR100(root=self.root, train=False, transform=self.transform, download=self.download,
                                 task_num=self.task_num, labels = self.labelType)
        test_task_datasets = [CifarDataset(data, self.transform) for data in
                              testDataset.task_datasets]  # the  test samples partitioned into 10 tasks
...
class myCIFAR100(datasets.CIFAR100):
    ...
    def __init__(
        ...
        self.labelType = labels #coarse or fine label
        if download:
            self.download()
